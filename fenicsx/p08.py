...
def mesh(tgl_xyz=None, tgl_len=None, save=False, show=False):
    gmsh.initialize()
    ...
    **0.5 )
    ...
    return tgl, nod, tgl_len

def main():
    tgl, nod, tgl_len = mesh()
    for i in range(20, -1, -1):
        #tgl_len *= 1.1
        tgl_len[np.argmin(nod.sum(axis=-1)[tgl[1]].min(axis=-1))] *= 0.5
        tgl, nod, tgl_len = mesh(nod[tgl[1]], tgl_len, i==0, i==0)
